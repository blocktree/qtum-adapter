...
func Test_case1(t *testing.T) {
	...
	/////////构建空交易单
	emptyTrans, err := CreateEmptyRawTransaction([]Vin{in1, in2}, []Vout{out}, lockTime, replaceable, QTUMTestnetAddressPrefix)
...
func Test_case4(t *testing.T) {
	...
	///////构建空交易单
	emptyTrans, err := CreateEmptyRawTransaction([]Vin{in}, []Vout{out}, lockTime, replaceable, QTUMTestnetAddressPrefix)
...
func Test_case5(t *testing.T) {
	...
	address, redeem, err := CreateMultiSig(required, [][]byte{pubA, pubB, pubC}, QTUMTestnetAddressPrefix)
	if err != nil {
		t.Error("创建多签地址失败！")
	} else {
		fmt.Println("地址为：")
